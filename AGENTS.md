...
- 优先使用 MaiBot Plugin SDK 的 `@LLMProvider`、`@Command`、`@Tool`、`@HookHandler` 等公开接口。
- 不要在插件代码中导入 `src.*`，保持插件可独立发布。
- 不要新增 `@Action`；新功能优先使用新版组件。
...
- Provider 只做协议转换和 DeepSeek 调用，不重写 MaiBot 已经构造好的人格、记忆和聊天上下文。
- DeepSeek Anthropic 兼容接口支持文本、工具调用、server web search；不要把图片或文档二进制直接发送给该接口。
- 开启思考模式时按 DeepSeek 文档使用 `thinking.type` 和 `output_config.effort`，不要主动传无效采样参数。
- 搜索来源默认只写日志和 `raw_data`，不主动附到聊天回复末尾。

参考文档：

- <https://docs.mai-mai.org/develop/plugin-dev/vibe-coding>
- <https://docs.mai-mai.org/develop/plugin-dev/llmprovider>
